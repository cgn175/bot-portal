...
func (m *Manager) ReadWorkspaceFile(ctx context.Context, agentID string, filename string) ([]byte, error) {
	...
	if filename == "config.toml" {
		workspacePath = fmt.Sprintf("%s/.zeroclaw/config.toml", ZEROCLAW_WORK_DIR)
	} else {
...
func (m *Manager) WriteWorkspaceFile(ctx context.Context, agentID string, filename string, content []byte) error {
	...
	if filename == "config.toml" {
		return fmt.Errorf("config.toml is read-only and automatically generated by the portal")
	}
	...
	if filename == "config.yaml" {
		workspacePath = fmt.Sprintf("/.zeroclaw/%s", filename)
		destDir = ZEROCLAW_WORK_DIR + "/.zeroclaw"
	} else {
		workspacePath = fmt.Sprintf("/workspace/%s", filename)
		destDir = ZEROCLAW_WORK_DIR + "/workspace"
	}
	...
	err = m.cli.CopyToContainer(ctx, cont.ID, destDir, tarReader, container.CopyToContainerOptions{
	...
	chownConfig := container.ExecOptions{
		Cmd:          []string{"chown", "claude:claude", ZEROCLAW_WORK_DIR + workspacePath},
		AttachStdout: false,
		AttachStderr: false,
	}
